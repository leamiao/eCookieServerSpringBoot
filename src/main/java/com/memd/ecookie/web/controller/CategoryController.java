...
public class CategoryController extends BaseEntityController<CategoryEntity> {
    ...
    protected List<CategoryEntity> searchEntities(HttpServletRequest request) {
        List<SearchParam> searchParams = JsonHelper.getInstance()
                .convertFromJsonToList(request.getParameter(Constants.FILTER), SearchParam.class);

        return this.categoryService.search(request.getParameter(Constants.SEARCH_TERM), searchParams,
